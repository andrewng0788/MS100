...
//				SetBTStartMove(TRUE);
				...
				DisplaySequence("BH - T Prepick to Pick ==> BOND_Z_WAIT_BT_READY_Q");
			}
			break;
...
*/
				...
				if (m_bPickDownFirst)	//	pick side down firstly
				{
					...
					else
					{
						...
						{
							CString szMsg = "";
							LONG lCompen_Z = 0;//OpCalCompen_Z(IsBHZ2ToPick());		//v4.53A25
							TakeTime(LC10);
							if (IsBHZ1ToPick())			//If BHZ1 towards PICK	
							{
								TakeTime(Z3);
		...
		case BOND_Z_WAIT_BT_READY_Q:
			...
			if (!WaitBTStartMoveWithoutDelay(0) && !m_bFirstCycle && m_bPickDownFirst)
			{
				...
				if  (WaitBTStartMove(200))
				{
					...
					SetBTStartMove(FALSE);
					m_qSubOperation = EJ_UP_Q;//PICK_Z2_DOWN_Q;	// -> normal flow
				}
				break;
				//====================================================================================
			 }
		...
		case WAIT_PICK_DELAY_Q:
			...
			//WaitForPickDelay(s_dTotalTime_Ej, lEjectorUpDelay, m_lPickDelay, s_dbStartZ2EjUp);
			Sleep(m_lPickDelay);
			TakeTime(LC10);
