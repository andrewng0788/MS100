...
INT	CWaferTable::OpMoveTable()
{
	...
	if (pApp->GetCustomerName() == CTM_SEMITEK && IsAutoRescanEnable()==FALSE && m_bPitchAlarmOnce )
	{
		if (HmiMessage_Red_Back("Pitch error happened, do you want to continue?", "Pitch Check", glHMI_MBX_CONTINUESTOP) == glHMI_CONTINUE)
		{
			if( Check201277PasswordWithWeek() )
			{
				SetErrorMessage("Pitch error happened, user to continue.");
...
*/
				...
				CString szTemp;
				szTemp.Format("WT MoveTable Normal map(%ld, %ld): Enc (%ld, %ld), Last-Enc (%ld, %ld), LF-Offset (%ld, %ld)", 
				...
				if (m_bMS60NewLFInLastCycle)
				{
					...
					if (IsBurnIn())
					{
						m_stLFDie.lX = -2 * lDiff_X * lDiePitchX_X;
						m_stLFDie.lY = -2 * lDiff_Y * lDiePitchY_Y;
			...
			{
				CString szTemp;
				szTemp.Format("use prescan coordinate, bDieLFed = %d, cur Map(%d,%d), LA(%d, %d), posn(%d, %d)", bDieLFed, 
					GetCurrentRow(), GetCurrentCol(), m_stLFDie.lWM_Y, m_stLFDie.lWM_X, lCalcX, lCalcY);
				CMSLogFileUtility::Instance()->MS60_Log(szTemp);
...
// prescan relative code	E
		...
		{
			CString szTemp;
			szTemp.Format("use prescan coordinate 2 case, bDieLFed = %d, cur Map(%d,%d), LA(%d, %d), posn(%d, %d)", bDieLFed, 
			GetCurrentRow(), GetCurrentCol(), m_stLFDie.lWM_Y, m_stLFDie.lWM_X, lCalcX, lCalcY);
			CMSLogFileUtility::Instance()->MS60_Log(szTemp);
...
// prescan relative code	E
	...
	if ((!bUseLF || !bDieLFed) && bGetPrescan)
	{
		...
		bPrescanJumpRotate = TRUE;
		bRotate = TRUE;
		szMoveMsg.Format("Get LAST %d,%d,T%d", lMoveWfX, lMoveWfY, lMoveWfT);
		CMSLogFileUtility::Instance()->WT_GetIdxLog(szMoveMsg);
	}
...
#else
		if (bThetaCorrection && m_lCountBurnInT >= 20)
#endif
		{
			bBurnInRotate = TRUE;		
			m_lStepBurnInT *= -1;
...
//	if( lIndexTime>BH_WT_MAX_MOTION_TIME )
	...
	CString szLog;
	szLog.Format("Move Table Die Position (ENC),%d,%d", m_stLFDie.lX, m_stLFDie.lY);
...
*/
						if (m_bUseMS60NewLFCycle)
						{
							...
							if ((lIndexTime + 5 - lBHZPickTime) > 0)
							{
								...
								szTemp.Format("m_bUseMS60NewLFCycle = TRUE, MegaDa WT Move #1, SleepTime=%d,lIndexTime=%d,lBHZPickTime=%d", (lIndexTime + 5 - lBHZPickTime), lIndexTime, lBHZPickTime);
								DisplaySequence(szTemp);
							}
							else
							{
								DisplaySequence("No MegaDa WT Move #1");
							}
						}
						else
						{
							if ((lIndexTime + 5 - lBHZPickTime) > 0)
							{
								Sleep(lIndexTime + 5 - lBHZPickTime);
								lIntervalTime = lIndexTime + 5 - lBHZPickTime;
								szTemp.Format("m_bUseMS60NewLFCycle = FALSE, MegaDa WT Move #1, SleepTime=%d,lIndexTime=%d,lBHZPickTime=%d", (lIndexTime + 5 - lBHZPickTime), lIndexTime, lBHZPickTime);
								DisplaySequence(szTemp);
							}
							else
							{
					...
					else
					{
						if ((lIndexTime - lSpeedUp) > 0)		//v4.22T1
						{
							...
							szTemp.Format("MegaDa WT Move #2, SleepTime=%d,lIndexTime=%d,lSpeedUp=%d", (lIndexTime - lSpeedUp), lIndexTime, lSpeedUp);
							DisplaySequence(szTemp);
							lIntervalTime = lIndexTime - lSpeedUp;
						}
						else
						{
							DisplaySequence("No MegaDa WT Move #2");
						}
					}
				}
				else	//v4.43T2
				{
					...
					{
						Sleep(lIndexTime - lSpeedUp);
						szTemp.Format("MegaDa WT Move #3, SleepTime=%d,lIndexTime=%d,lSpeedUp=%d", (lIndexTime - lSpeedUp), lIndexTime, lSpeedUp);
						DisplaySequence(szTemp);
						lIntervalTime = lIndexTime - lSpeedUp;
					}
					else
					{
						DisplaySequence("No MegaDa WT Move #3");
					}
				}
...
//					TakeData(LC4, lSpeedUp); 2018.3.5
				...
				if (m_bUseMS60NewLFCycle) //only for MS50
				{
					if (lIntervalTime < lIndexTime - 7)
					{
						...
						szTemp.Format("MegaDa WT Move #4, SleepTime=%d,lIndexTime=%d,lIntervalTime=%d", (lIndexTime - 7 - lIntervalTime), lIndexTime, lIntervalTime);
						DisplaySequence(szTemp);
					}
					else
					{
						DisplaySequence("No MegaDa WT Move #4");
					}
					if ( GetCurrDieEAct() == WAF_CDieSelectionAlgorithm::PICK )
					{
						if( !IsLayerPicking() )
			...
			else
			{
				...
				DisplaySequence("MegaDa WT Move #5, No Pick Action wait for compensation");
			}
		}
#endif
...
INT CWaferTable::OpCompensation(BOOL bGoodDie)	// for wait die ready (pr done) to do move direct or indirectly
{
	...
	if ((!m_bPerformCompensation) || (bLFNoComp) || IsES101() || IsES201())		//v2.96T3	//v4.28
	{
		...
		if (m_bUseLF2ndAlignment)
		{
			...
			if (stInfo.bStatus == TRUE) {
				CString szLog;
				szLog.Format("WT - LF 2nd Alignment Result (%d) - Offset (%ld, %ld)",  stInfo.bStatus, stInfo.lX, stInfo.lY);
				DisplaySequence(szLog);
				...
				//X_Profile(NORMAL_PROF);
				//Y_Profile(NORMAL_PROF);
				//X_MoveTo(m_lEnc_X + stInfo.lX, SFM_NOWAIT);
				//Y_MoveTo(m_lEnc_Y + stInfo.lY, SFM_NOWAIT);
				//Sleep(30);
			}
	...
	else if (bBinTableThetaCorrection)
	{
		...
		if (GetCurrDieEAct() == WAF_CDieSelectionAlgorithm::PICK)
		{
			...
			CString szLog;
			szLog.Format("WT - m_lSetThetaCounter = %d, Compansation Theta = %f", m_lSetThetaCounter, dTheta);
			DisplaySequence(szLog);
			m_lSetThetaCounter++;
		}
		else
		{
			...
			CString szLog;
			szLog.Format("WT - m_lSetThetaCounter (NOT PICK) = %d, Compansation Theta = %f", m_lSetThetaCounter, dTheta);
			DisplaySequence(szLog);
			m_lSetThetaCounter++;
		}
	}
